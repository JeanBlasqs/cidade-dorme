...
function assignRolesWithDiversity(players, counts) {
  // Sorteio totalmente aleatório a cada partida.
  // A única proteção é validar que a distribuição final respeita exatamente
  // a quantidade configurada de cada papel.
  const pool = [
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    // Sanidade: confirma que cada papel apareceu exatamente a quantidade
    // configurada. Se alguma inconsistência ocorrer, tenta novamente.
    const assignedCounts = assignments.reduce((result, { role }) => {
      result[role] = (result[role] || 0) + 1;
      return result;
    }, {});

    const valid = Object.entries(counts).every(
      ([role, count]) => (assignedCounts[role] || 0) === Number(count),
    );
