...
function assignRolesWithDiversity(players, counts) {
  // O sorteio continua 100% aleatório a cada partida.
  // A única proteção aqui é garantir que a distribuição respeite exatamente
  // a quantidade configurada de cada papel. Se por algum motivo uma tentativa
  // inválida acontecer, sorteamos novamente.
  const pool = [
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    const roleTotals = assignments.reduce((totals, { role }) => {
    ...
    const valid = [
      ...
      "anjo",
      "cidadao",
    ].every((role) => (roleTotals[role] || 0) === (counts[role] || 0));
...
async function hostStartGame() {
  ...
  const roleAssignments = assignRolesWithDiversity(players, counts);
